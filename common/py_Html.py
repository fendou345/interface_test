...
<h1>接口测试的结果</h1>'''
def shouye(starttime,endtime,passge,fail,excepthions,unknown):
    beijing='''
    <table  class="table table-hover table-condensed">
		...
		<td><strong>结果:</strong>
			...
			Fail: <strong >%s</strong>
			   ...
			   </div> '''%(starttime,endtime,(endtime-starttime),passge,fail,excepthions,unknown)
    return beijing
...
def passfail(tend):
    ...
    return htl
def ceshixiangqing(reslt,id,name,url,meth,header,data,yuqi,json,relust):
    xiangqing='''
        <tr class="case-tr %s">
            <td>%s</td>
            <td>%s</td>
    ...
    '''%(reslt,id,name,url,meth,header,data,yuqi,json,passfail(relust))
    return xiangqing
...
def relust(titles,starttime,endtime,passge,fail,id,name,url,meth,header,data,yuqi,json,relust,exceptions,unknown):
    if type(name) ==list:
        relus=' '
        for i in range(len(name)):
            if relust[i] == "pass":
                ...
                clazz = "warning"
            elif relust[i]== "unknown":
                clazz = "danger"
            else:
                clazz='error'
            relus+=(ceshixiangqing(clazz,id[i],name[i],url[i],meth[i],header[i],data[i],yuqi[i],json[i],relust[i]))
        text=title(titles)+connent+shouye(starttime,endtime,passge,fail,exceptions,unknown)+shanghai+relus+weibu
    else:
        text=title(titles)+connent+shouye(starttime,endtime,passge,fail,exceptions,unknown)+shanghai+ceshixiangqing('error',id,name,url,meth,header,data,yuqi,json,relust)+weibu
    return text
def createHtml(filepath,titles,starttime,endtime,passge,fail,id,name,url,meth,header,data,yuqi,json,relusts,exceptions,unknown):
    texts=relust(titles,starttime,endtime,passge,fail,id,name,url,meth,header,data,yuqi,json,relusts,exceptions,unknown)
